...
The app uses a custom HTML tooltip (`components/Visualization/CustomTooltip/CustomTooltip.tsx`) instead of Observable Plot's built-in SVG tooltips. Tooltip content is provided via a hidden `__customTooltip` config attached to plot options and handled by `components/Visualization/ObservablePlot.tsx`.

Highlights:
- Tooltip fields are generated by `createTooltipFieldsGetter()` in `observable-plot-generator/utils/tooltipUtils.ts`.
- Hover listeners are attached to SVG marks; the hovered datum is used to build tooltip rows.
- A color swatch appears at the top of the tooltip, showing the exact color of the hovered chart element. The swatch color is read directly from the hovered element's `fill` (or computed `fill`), avoiding DOM traversal while matching Observable Plot's color encoding precisely.
