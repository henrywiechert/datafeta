...
describe('detectDefaultUserChartType (PR 10 — consolidated source of truth)', () => {
  ...
  describe('heatmap rule', () => {
    ...
    test('does NOT route to heatmap when color is a dimension (all-discrete → table)', () => {
      expect(
        detectDefaultUserChartType([dim('region')], [dim('product')], dim('segment'))
      ).toBe('table-refactor');
    });

    test('does NOT route to heatmap when X has more than one discrete dim (all-discrete → table)', () => {
      // No continuous field on either axis → Tableau-style table.
      expect(
        detectDefaultUserChartType(
  ...
  describe('single-axis / dim-only fallbacks', () => {
    ...
    test('discrete dim on X only, no measures → table', () => {
      expect(
