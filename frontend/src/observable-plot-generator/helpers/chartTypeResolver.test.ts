...
describe('detectDefaultUserChartType (PR 10 — consolidated source of truth)', () => {
  ...
  describe('heatmap rule', () => {
    ...
    test('does NOT route to heatmap when there is no color field (falls through to table-refactor for all-discrete shape)', () => {
      expect(
        detectDefaultUserChartType([dim('region')], [dim('product')], null)
      ).toBe('table-refactor');
    });

    test('does NOT route to heatmap when color is a dimension (falls through to table-refactor for all-discrete shape)', () => {
      expect(
    ...
    test('does NOT route to heatmap when X has more than one discrete dim', () => {
      // No continuous candidates on either axis, no measures → table-refactor fallback.
      expect(
        detectDefaultUserChartType(
