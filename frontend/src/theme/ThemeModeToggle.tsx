 ...
 * dark mode and two are the light mode, differing only in which colour scheme
 * that mode uses — which is exactly the split `setColorScheme` exists for.
 */
type Choice = 'system' | 'light' | 'solarized' | 'dark' | 'dim';
...
const CHOICES: Array<{ id: Choice; label: string; icon: React.ReactElement }> = [
  ...
  { id: 'solarized', label: 'Solarized', icon: <WbTwilightIcon fontSize="small" /> },
...
/**
 * Theme switch: light, solarized, dark, dim, or follow-system.
 *
 ...
 *
 * Each mode has more than one scheme behind it — dark/dim, light/solarized —
 * so an entry sets both: `setMode('dark')` and `setColorScheme({ dark })`. A
 * consequence worth knowing: the scheme for each mode is remembered
 * independently of the mode, so picking Dim and later Follow system means a
 * system that reports dark gets dim.
 *
 * It was a 3-state cycling button while there was only one scheme per mode.
 * Five states is well past where cycling is a shortcut, and a menu can show the
 * names.
 */
const ThemeModeToggle: React.FC = () => {
  ...
  // Both modes now have two schemes behind them, so the entry is the mode plus
  // which scheme that mode resolves to.
  ...
  const choose = useCallback((id: Choice) => {
    if (id === 'dark' || id === 'dim') {
      ...
      setMode('dark');
    } else if (id === 'light' || id === 'solarized') {
      setColorScheme({ light: id });
      setMode('light');
    } else {
      setMode(id);
    }
