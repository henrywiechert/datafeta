# loose collection of things still to be implemented

[ ] Linear Regression lines on color legend 'select'
- those lines do not vanish separately
...
- also for cases where no cont. field is present

## [ONGOING] Reference / Trend lines / Bands
- linear regression - Done
- moving average - Done
- more helpers still to come
  - bollinger bands removed (no use of it)
  - clustering ?
...
## [DONE] Consistent Naming for aggregates in all panels and tooltip
...
## [DONE] Filter (or Zoom) facet by right click into facet
- filter and zoom available

## [DONE] UI driven filters (right click on data point, show all involved filterable categories, select the ones that shall match to selection)
- DONE
...
- not really needed, as now facet labels can be formatted (direction, size etc.)

## [DONE] Add legend stack to fullscreen view
...
- SQL like search does not work for thousands of elements

## [] Gantt chart width ends with start of last event
- too short GANTT chart
...
## [] Undo across sheets
- remember Undo per sheet, also when sheet changes
- doesn't it work now ??
