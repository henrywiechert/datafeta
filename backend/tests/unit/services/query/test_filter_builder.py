...
def test_not_like_filter_builds_negated_like_sql():
    qs = QueryService()
    desc = QueryDescription(
        target_table="sales",
        dimensions=[Dimension(field="category", flavour="discrete")],
        filters=[Filter(field="category", operator="not like", value="%Books%")],
    )
...
def test_not_ilike_filter_builds_negated_ilike_sql():
    qs = QueryService()
    desc = QueryDescription(
        target_table="sales",
        dimensions=[Dimension(field="category", flavour="discrete")],
        filters=[Filter(field="category", operator="not ilike", value="%books%")],
    )
